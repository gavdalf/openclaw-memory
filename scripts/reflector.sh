#!/usr/bin/env bash
# reflector.sh — Consolidates observations when they grow too large
...
Preserve all 🔴 high-priority items. Compress 🟡 medium items. Summarise 🟢 low items.
...
log "Reflection complete: $WORDS → $NEW_WORDS words ($(( (WORDS - NEW_WORDS) * 100 / WORDS ))% reduction)"
